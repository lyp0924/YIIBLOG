...
class ArticleSort extends Model{
	...
	public function article_insert($data=array()){
		...
		return $this->ArticleSortModel->insert($data);
			
	}
	
	public function article_delete($id){
		return $this->ArticleSortModel->delete($id);
	}
	
	public function article_update($sid,$data=array()){
		return $this->ArticleSortModel->where('sid', $sid)->update($data);
	}
		
	public function article_select(){
		return $this->ArticleSortModel->where('status', '1')->select();
	}
	
	public function article_find($id){
		return $this->ArticleSortModel->find($id);
	}
}
