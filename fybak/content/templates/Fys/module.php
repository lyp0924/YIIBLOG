...
function blog_navi(){
 ...
 <ul class="sub-menu">
                <?php foreach ($value['children'] as $row){
                        echo '<li><a href="'.Url::sort($row['sid']).'"><i class="'.$arr_navico1[$row['sid']].'"></i>'.$row['sortname'].'</a></li>';
                }?>
			</ul>
            <?php endif;?>
...
function blog_comments_post($logid,$ckname,$ckmail,$ckurl,$verifyCode,$allow_remark){
	...
	<div class="comment_post_wrap comment_post comment-open" id="comment-post" >
		...
		<form method="post" name="commentform" id="commentform" action="<?php echo BLOG_URL; ?>index.php?action=addcom">
			...
			<div class="comment_user_info"><div id="loging"></div>
